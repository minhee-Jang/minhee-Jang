...
class REDS(SRData):
    ...
    def _set_filesystem(self, data_dir):
        self.apath = os.path.join(data_dir, 'reds')

        if self.is_train and not self.is_valid:
            self.dir_hr = os.path.join(self.apath, 'train', 'train_sharp')
            self.dir_lr = os.path.join(self.apath, 'train', 'train_blur_comp')
        elif self.is_train and self.is_valid:
            self.dir_hr = os.path.join(self.apath, 'val', 'val_sharp')
            self.dir_lr = os.path.join(self.apath, 'val', 'val_blur_comp')
        else:
            self.dir_hr = os.path.join(self.apath, 'test', 'test_sharp')
            self.dir_lr = os.path.join(self.apath, 'test', 'test_blur_comp')
