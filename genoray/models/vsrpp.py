"""
https://mmediting.readthedocs.io/en/latest/api.html#mmedit-models
This model use mmedit, which means that this can be run only on Ubuntu environment
"""
...
class VSRPP(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--loss', type=str, default='charbonnier',
            help='choose loss criterion [l2|charbonnier]')
        parser.add_argument('--mid_channels', type=int, default=64,
            help='number of mid channels')
        parser.add_argument('--num_blocks', type=int, default=7,
            help=' The number of residual blocks in each propagation branch')
        parser.add_argument('--max_residue_magnitude', type=int, default=7,
            help='The maximum magnitude of the offset residue')
        # parser.add_argument('--is_low_res_input', type=bool, default=False,
        #     help='Whether the input is low-resolution or not. \
        #         If False, the output resolution is equal to the input resolution. Default: True.')
        parser.add_argument('--cpu_cache_length', type=int, default=100,
            help='When the length of sequence is larger than this value,\
                the intermediate features are sent to CPU. This saves GPU memory, but slows down the inference speed. \
                You can increase this number if you have a GPU with large memory. Default: 100.')
        parser.add_argument('--spynet_pretrained', type=str, default=None,
            help='spynet path')
    ...
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            elif opt.loss == 'charbonnier':
                self.loss_criterion = CharbonnierLoss()
    ...
    def set_input(self, input):
        self.x = input['lr'].to(self.device).transpose(1, 2)
        if 'hr' in input:
...
from .vsrbasic import (ResidualBlocksWithInputConv, SPyNet)
# from mmedit.models.common import PixelShufflePack, flow_warp
from models.mmedit.models.common.upsample import PixelShufflePack
...
class BasicVSRPlusPlus(nn.Module):
    ...
    def __init__(self, opt):
        ...
        max_residue_magnitude = opt.max_residue_magnitude

        is_low_res_input = True if opt.scale > 1 else False
        self.is_low_res_input = is_low_res_input
        # is_low_res_input = opt.is_low_res_input
        self.cpu_cache_length = opt.cpu_cache_length
        cpu_cache_length = opt.cpu_cache_length

        # optical flow
        self.spynet = SPyNet(pretrained=opt.spynet_pretrained)
        ...
        self.img_upsample = nn.Upsample(
            scale_factor=opt.scale, mode='bilinear', align_corners=False)
