...
class REDSQP2(SRData):
    ...
    def _set_filesystem(self, data_dir):
        ...
        if self.is_train and not self.is_valid:
            ...
            self.dir_lr = os.path.join(self.apath, 'train', 'lr_qp37_down2')
        elif self.is_train and self.is_valid:
            self.dir_hr = os.path.join(self.apath, 'val', 'hr')
            self.dir_lr = os.path.join(self.apath, 'val', 'lr_qp37_down2')
        else:
            self.dir_hr = os.path.join(self.apath, 'test', 'hr')
            self.dir_lr = os.path.join(self.apath, 'test', 'lr_qp37_down2')
