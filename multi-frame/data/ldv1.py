...
class LDV1(SRData):
    ...
    def _set_filesystem(self, data_dir):
        self.apath = os.path.join(data_dir, 'ldv2022', 'track1')
        # self.apath = os.path.join(data_dir, 'ldv2022', 'track1_test')


        if self.is_train:
            self.dir_hr = os.path.join(self.apath, 'train_gt')
            self.dir_lr = os.path.join(self.apath, 'train_QP37')
        else:
            if self.report_ldv:
                self.dir_lr = os.path.join(self.apath, 'validation_track1')
            else:
                self.dir_hr = os.path.join(self.apath, 'test_fake_gt')
                self.dir_lr = os.path.join(self.apath, 'test_fake_QP37')
