...
def create_dataset(opt):
    dataloader = {}
    if opt.is_train and opt.valid_ratio != 0:
        # dataloader['train'] = TrainDataloader(opt).train_dataloader
        dataloader['train'], dataloader['test'] = TrainDataloader(opt).get_datasets()
    elif opt.is_train and opt.valid_ratio == 0:
        dataloader['train'], _ = TrainDataloader(opt).get_datasets()
        dataloader['test'] = TestDataloader(opt).test_dataloaders[0]
    else:
